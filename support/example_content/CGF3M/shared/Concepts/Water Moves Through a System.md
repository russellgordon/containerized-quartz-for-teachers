...
```mermaid
graph LR
    O["Ocean and lake"] -->|evaporation| A["Atmosphere"]
    V["Vegetation"] -->|transpiration| A
    A -->|precipitation| S["Land surface"]
    S -->|infiltration| G["Ground water"]
    S -->|overland flow| R["River"]
    G -->|base flow| R
    R --> O
```
