...
quality in an entry comes down to one word: **specific**. A strong
entry could only have been written by you, about this program, on this
day. A weak one could have been written by anyone, about any
programming class, from the hallway.
...
same day, remembered properly — a line number, an error, a cause, a
sentence somebody actually said. And notice that the strong versions
are barely longer. Specific is not the same as long; it is the
difference between "it broke" and a bug you could reproduce, which is
exactly what the "what broke" prompt in [[Code Journal]] is asking
for.
...
The feeling is real data about you; the error is the handle you can
turn. Pairing them is exactly what [[Mistakes Are Data]] argues
staying curious depends on.
