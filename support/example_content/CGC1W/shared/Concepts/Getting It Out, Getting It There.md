...
```mermaid
graph TD
    A["A deposit is found"] --> B{"How deep is it?"}
    B -- "Near surface" --> C["Open pit: cheaper per tonne,<br/>large surface footprint"]
    B -- "Deep" --> D["Shaft or ramp: costlier per tonne,<br/>smaller footprint"]
    C --> E{"Can it reach road, rail, or port?"}
    D --> E
    E -- "Yes" --> F["Develop"]
    E -- "No" --> G["Stays in the ground until<br/>transport is worth building"]
```
