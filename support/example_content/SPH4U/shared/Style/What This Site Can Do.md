...
> [!success]- Answer: the minimum speed at the top of an 8.0 m loop
> $v = \sqrt{gr} = \sqrt{(9.8)(8.0)} = 8.9\ \text{m/s}$, and the mass
...
Inline maths sits inside a sentence: a satellite at
$7.0 \times 10^6\ \text{m}$ from Earth's centre orbits at
$v = 7.5\ \text{km/s}$.

Display maths gets a line of its own:

$$F_c = \frac{mv^2}{r} = \frac{4\pi^2 mr}{T^2}$$
...
| `$\gamma = \frac{1}{\sqrt{1 - v^2/c^2}}$` | $\gamma = \frac{1}{\sqrt{1 - v^2/c^2}}$ | Nested fractions and radicals |
...
```mermaid
graph LR
    A["Mass"] -->|source of| B["Gravitational field"]
    C["Charge"] -->|source of| D["Electric field"]
    E["Moving charge"] -->|source of| F["Magnetic field"]
    B --> G["Force on a test object"]
    D --> G
    F --> G
```
...
- [ ] Laser below shoulder height
- [ ] Full circle clear before the pig flies
- [x] Axes chosen and written down
...
- A plain link: [[Centripetal Force]]
- A link with different words:
  [[Centripetal Force|the job description, not a new force]]
