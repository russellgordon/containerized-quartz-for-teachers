---
title: Unit 3, Day 1
draft: false
created: __CREATED_CLASS_14__
transcludeTitleSize: h2
enableToc: false
excludeBacklinks: true
tags:
  - unit-3
---
## Agenda

1. Random groups — [[The Slope Detective]] at the boards: the graph
   you are given is $f'$; the function that made it has gone missing
2. Discuss: how much of $f$'s story can the witness pin down — and how
   many innocent functions fit the evidence?
3. Consolidate: the sign of $f'$ tells direction, the zeros of $f'$
   mark the turns — and one thing is unrecoverable
4. Sketch-off: two candidate graphs of $f$ per group, both defensible

## Things to do before our next class

- [ ] Journal: a detective story in one paragraph — what $f'$ saw,
      and what it could never have seen.
