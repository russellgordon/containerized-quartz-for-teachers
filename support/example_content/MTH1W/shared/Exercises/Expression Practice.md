---
title: Expression Practice
publish: true
created: __CREATED__
tags:
  - exercises
---
These questions follow [[Algebraic Expressions]] — the habit of naming
a pattern with algebra was built in class through [[Visual Patterns]]
and it powers our [[Pattern Machines]] project. Attempt each question
before expanding its answer.

## Questions

1. Simplify $3x + 5 + 2x - 1$.
2. Expand $4(2n + 3)$.
3. Are $2(n + 3)$ and $2n + 3$ equivalent? Test both with $n = 5$,
   then explain what the test tells you — and what it cannot tell you.
4. Two students describe the same growing pattern. Maya writes
   $4n + 2$; Theo writes $2(2n + 1)$. Are they describing the same
   pattern? Justify your answer two different ways.
5. Simplify $5a + 2b - 3a + b$.
6. **Find the error.** Lena expands $3(x + 2)$ and gets $3x + 2$.
   What did the 3 fail to do, and how could substituting $x = 0$ have
   caught it?
7. **Challenge.** A rectangle has width $w$ and length $w + 5$. Write
   two different-looking expressions for its perimeter, then show
   they are equivalent.

## Answers

> [!success]- Answer 1
> Collect like terms: $3x + 2x = 5x$ and $5 - 1 = 4$, giving
> $5x + 4$. Only terms measuring the same thing can combine.

> [!success]- Answer 2
> The 4 multiplies *both* terms inside: $8n + 12$. Picture four
> copies of a strip holding $2n + 3$ tiles.

> [!success]- Answer 3
> At $n = 5$: $2(5+3) = 16$ but $2(5) + 3 = 13$ — not equivalent,
> and one failed test settles it. Careful, though: a *match* would
> prove nothing — agreeing at one value is not agreeing at all values.

> [!success]- Answer 4
> Numerically, $n = 1$ gives 6 and 6; $n = 2$ gives 10 and 10.
> Algebraically, expanding Theo's: $2(2n + 1) = 4n + 2$ — identical
> to Maya's. Same pattern, two honest descriptions of it.

> [!success]- Answer 5
> $5a - 3a = 2a$ and $2b + b = 3b$, so $2a + 3b$. The $a$s and $b$s
> stay separate — they count different things.

> [!success]- Answer 6
> The 3 multiplied the $x$ but skipped the 2; correct is $3x + 6$.
> At $x = 0$: $3(0 + 2) = 6$ while Lena's $3(0) + 2 = 2$. One
> substitution, error exposed — test equivalence, don't trust it.

> [!success]- Answer 7
> Adding the sides: $w + (w + 5) + w + (w + 5)$. Doubling the pair:
> $2w + 2(w + 5)$. Both simplify to $4w + 10$ — the geometry did not
> change, only the story told about it.

%%curriculum-start%%
## Curriculum connection

![[C1.3]]

![[C1.4]]
%%curriculum-end%%
