...
tags:
  - exercises
---
These questions follow [[Powers and Exponent Rules]] and reach into
[[Scientific Notation]] — the sheer scale of powers first hit home in
class during [[How Big Is a Million]]. Attempt each before expanding.

## Questions

1. Evaluate $2^5$, $(-3)^2$, and $-3^2$ — and explain why the last
   two are different.
2. Continue the pattern $3^3 = 27$, $3^2 = 9$, $3^1 = 3$, and use it
   to find $3^0$ and $3^{-1}$. What is the pattern doing at each step?
3. Write $150{,}000{,}000$ km (the distance from Earth to the Sun)
   in scientific notation, and write $4.5 \times 10^{-3}$ as a
   decimal.
4. Simplify, leaving each as a single power: $x^4 \cdot x^3$ and
   $\frac{2^5}{2^2}$.
5. Simplify $(a^2 b)^3$.
6. **Find the error.** Priya writes $2^3 \cdot 2^4 = 4^7$. What rule
   was misapplied, and what is the correct simplification?
7. **Challenge.** Which is larger, $2^{100}$ or $10^{30}$? Decide
   without a calculator, using $2^{10} = 1024 \approx 10^3$.
