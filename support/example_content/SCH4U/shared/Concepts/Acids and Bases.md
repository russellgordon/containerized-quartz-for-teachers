...
$\text{H}^+$ in water, a base produces $\text{OH}^-$. It works, in
...
- **Acidity becomes a relationship, not a property.** A substance is an
  ...
  $\text{HCO}_3^-$ and $\text{H}_2\text{PO}_4^-$.
...
ion**, $\text{H}_3\text{O}^+$. This course writes
$[\text{H}_3\text{O}^+]$ throughout, because the whole picture is about
...
$\text{CH}_3\text{COO}^-$, is its **conjugate base**. Water accepted the
proton, so it is the base; $\text{H}_3\text{O}^+$ is its **conjugate
acid**.

A conjugate pair differs by **exactly one proton** — one $\text{H}$, and
...
in water and, for practical purposes, no $\text{HCl}$ molecules remain.
...
| Solution | What is present | $[\text{H}_3\text{O}^+]$ | pH |
| --- | --- | --- | --- |
| Hydrochloric acid | ions only, no intact $\text{HCl}$ | 0.10 mol/L | 1.00 |
...
$$\text{pH} = -\log[\text{H}_3\text{O}^+] \qquad \text{pOH} = -\log[\text{OH}^-] \qquad \text{pH} + \text{pOH} = 14.00$$
...
$[\text{H}_3\text{O}^+] = [\text{OH}^-]$, not "pH is 7" — and as $K_w$
