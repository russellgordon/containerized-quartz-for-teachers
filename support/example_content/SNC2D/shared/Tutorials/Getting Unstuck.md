...
```mermaid
graph TD
    A["Stuck"] --> B{"Can you say what the question is asking?"}
    B -->|no| C["Re-read it aloud. Underline the quantity wanted"]
    B -->|yes| D{"Do you know a worked example like it?"}
    D -->|no| E["Open the concept page. Find the nearest case"]
    D -->|yes| F{"Where exactly does yours differ?"}
    F -->|one step| G["Do that step alone, on its own"]
    F -->|everywhere| H["Ask — you are missing an idea, not a step"]
```
