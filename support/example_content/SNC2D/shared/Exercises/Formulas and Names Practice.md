...
tags:
  ...
  - exercises
---
A formula is a sentence in a very compressed language. These questions
are translation practice, both ways. Keep a periodic table and a
polyatomic ion list beside you — that is how a chemist works, and it is
...
> states the subscripts, because the charges already determine them.

**2.** Write the formula for each: magnesium nitride, potassium
...
> pair of elements can combine in several different ratios that are all
> perfectly stable. Nitrogen and oxygen alone give a whole family of
> them. Since the ratio is not determined by anything, the name has to
> state it, and that is what the prefixes are for.
>
> In an **ionic** compound the charges fix the ratio, so stating it
> would be redundant. Note also that *mono-* is dropped on the first
...
>
> Notice the naming pattern too: an acid whose negative ion ends in
> *-ate* gives an *-ic* acid, so sulfate becomes sulfuric and nitrate
> becomes nitric.
...
>
> Where an ionic compound genuinely is ambiguous — a metal with more
> than one possible charge — the fix is a Roman numeral giving the
> **charge**, as in iron(III) chloride, not a prefix giving the count.
> The student has spotted a real ambiguity problem and reached for the
> wrong tool.
>
...
are what you will be balancing in [[Balancing Practice]].
