---
title: Unit 3, Day 1
draft: false
created: __CREATED_CLASS_14__
transcludeTitleSize: h2
enableToc: false
excludeBacklinks: true
tags:
  - unit-3
---
## Agenda

1. Warm-up: [[Spot the Hazard]] — a heater with a timer and no
   thermostat
2. Bench: hold a small enclosure at a target temperature using a timer
   and nothing else. Predict how far it will drift in ten minutes,
   then let it drift and record the truth
3. Now put a thermometer in the loop and make yourself the controller.
   Same target, different result — say precisely what changed
4. Name it: [[Open and Closed Loop Control]] — the difference is
   whether the output is allowed to affect the input
5. Log both drifts, measured, in your [[Tech Journal]]

## Things to do before our next class

- [ ] Write the control rule you used as a human controller in three
      lines a machine could follow.
- [ ] Journal: one system in the building that is open loop, and what
      it costs somebody.
