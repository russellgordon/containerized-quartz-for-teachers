...
1. **A Preview item in a menu bar** (rather than only the section window's own
   button). Not found anywhere in `windows-app/Plantoir` as of 2026-08-22.
...
4. **A Windows screenshot-capture harness for plantoir.app** (entry 255) —
   explicitly deferred until the Windows app is worth photographing; see
   "plantoir.app is generated…" below for what it will owe.

5. **The salvaged capture-dialog fixes on `issue/windows-capture-dialog-fixes`
...
6. **Two things to measure, not copy, on real Windows hardware** (see "Two
...
7. **The working-folder path bar's fuller gesture set** (double-click to open,
...
8. **The first-deploy marker's destination-scoping** (`AssistWorkspace.cs`
